...
private let mockDbQuery: osr_db_query_fn = { sqlPtr, paramsPtr in
    ...
    if sql.contains("JOIN messages m ON m.thread_id = t.thread_id") && sql.contains("WHERE m.message_id = ?1") {
    ...
    if sql.contains("FROM threads") && sql.contains("ORDER BY updated_at DESC LIMIT") {
        ...
        if sql.contains("labels LIKE") {
            let labelIdx = sql.contains("LIKE ?1") && !sql.contains("LOWER(participants)") ? 0 :
                           sql.contains("LIKE ?2") ? 1 : -1
    ...
    if sql.contains("SELECT COUNT(*)") && sql.contains("direction = 'out'") && sql.contains("LOWER(to_address) LIKE") {
    ...
    if sql.contains("SELECT COUNT(*)") && sql.contains("FROM threads") && sql.contains("LOWER(participants) LIKE") {
        guard let search = paramValues.first as? String else { return returnRows([[0]]) }
        let cleaned = search.replacingOccurrences(of: "%", with: "").lowercased()
    ...
    if sql.contains("SELECT COUNT(*)") && sql.contains("direction = 'out'") && sql.contains("created_at >= ?2") {
        guard let tid = paramValues.first as? String,
              let since = paramValues[safe: 1] as? Int else { return returnRows([[0]]) }
        let count = mockMessages.filter {
            ($0["thread_id"] as? String) == tid &&
            ($0["direction"] as? String) == "out" &&
