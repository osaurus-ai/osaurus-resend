...
struct ToolTests {
    ...
    func replyAttachesArtifacts() {
        ...
        ctx.taskArtifacts["task-art"] = [
            CollectedArtifact(filename: "report.pdf", data: Data("pdf-content".utf8), mimeType: "application/pdf")
        ]
    ...
    func listThreadsAll() {
        MockHost.setUp()
        DatabaseManager.initSchema()
        DatabaseManager.createThread(threadId: "t-l1", subject: "Thread 1", participants: ["a@a.com"], messageId: nil, refs: nil)
        DatabaseManager.createThread(threadId: "t-l2", subject: "Thread 2", participants: ["b@b.com"], messageId: nil, refs: nil)
    ...
    func listThreadsByParticipant() {
        MockHost.setUp()
        DatabaseManager.initSchema()
        DatabaseManager.createThread(threadId: "t-p1", subject: "Alice", participants: ["alice@a.com"], messageId: nil, refs: nil)
        DatabaseManager.createThread(threadId: "t-p2", subject: "Bob", participants: ["bob@b.com"], messageId: nil, refs: nil)
    ...
    func listThreadsByLabel() {
        MockHost.setUp()
        DatabaseManager.initSchema()
        DatabaseManager.createThread(threadId: "t-lb1", subject: "Urgent", participants: ["a@a.com"], messageId: nil, refs: nil)
        DatabaseManager.updateThread(threadId: "t-lb1", labels: ["urgent"])
        DatabaseManager.createThread(threadId: "t-lb2", subject: "Normal", participants: ["b@b.com"], messageId: nil, refs: nil)
    ...
    func labelThreadAddAndRemove() {
        ...
        let tool = ResendLabelThreadTool()
        let result = tool.run(args: "{\"thread_id\":\"t-both\",\"add\":[\"new_label\"],\"remove\":[\"old_label\"]}")
